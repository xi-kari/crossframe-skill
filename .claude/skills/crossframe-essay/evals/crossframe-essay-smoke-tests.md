...
- 是否标明 `full-visible-v5-longform`，并真的输出完整可见底稿和完整长文正文。
...
- 来源台账是否通过十字段硬校验，时间可复核、使用位置可定位，没有合并字段或伪填字段。
...
- 底稿是否包含 `命题台账摘要`，并列出至少 1-3 个关键 `claim_id`。
- 正文中心命题、机制句、高风险概念句、行动建议、概率排序和点睛句是否能回指 `claim_id`。
- 写作技法是否只绑定已有 `claim_id`，没有新增事实、强判断或隐喻证明。
- 若正文有无 `claim_id` 的强句子，是否删除、降档或标为表达转译。
...
- 来源台账逐条写出source_id、来源、时间、来源类型、支持的 claim_id / 命题、不能证明什么、证据档位、使用位置、降档理由、仍需补证处。
- 不把自评报告或截图写成已核验事实。
