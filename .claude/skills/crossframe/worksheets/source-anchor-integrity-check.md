...
| 命题台账 | 每条 `claim_id` 必须回指事实、源锚点、机制候选、概念契约、判断档位、撤回条件或来源台账 | 缺失时不得进入正文；删除、补台账、降档，或标为本文推断 |
...
仅写一行 `source_grounding` 或在质量闸里自称“源锚点检查通过”，不算已执行。本检查至少要可见摘要七项：中心命题、命题台账、机制候选、高风险概念、行动边界、文章类型转译、写作技法边界；并列出无法回指内容和降档决定。
...
| 正文短摘 | 类型 | claim_id | 胶囊中是否已有锚点 | 概念契约状态 | 处理 |
| --- | --- | --- | --- | --- | --- |
|  | 高风险概念 / 机制句 / 行动建议 / 概率排序 / 点睛句 | CL / 无 | 是 / 否 | pass / partial / fail / 不适用 | 保留 / 补读 / 删除 / 降档 / 标为表达转译 |

若正文新增高风险概念、机制句、行动建议、概率排序或点睛句，但没有对应 `claim_id`，或 `claim_id` 无法回指胶囊 source grounding、来源台账或概念契约，不得用“文章表达需要”放行；必须补台账、补读、删除、降档或标为“表达转译 / 本文推断 / 外部思想映射”。
...
| claim ledger 缺失 | 不进入文章正文、公共发布、强判断或 `substantive_pass` |
| 正文命题不在 claim ledger | 删除、补台账、降档，或标为本文推断 / 表达转译 / 外部思想映射 |
