...
本 trial 是反例压力测试：如果 v6 半量化表被用来给“更大格局解释”背书，它必须主动降档并阻断。关键不是给关系打分，而是保护低尺度痛苦、责任链和可撤回行动边界。
...
本案例不能证明这段关系一定有害，不能证明成长意义不存在，不能证明任何一方应被公开定性。它只证明一个 v6 治理要求：半量化工具必须能阻断“用尺度解释取消低尺度痛苦”的误用。
