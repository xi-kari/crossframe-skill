...
- 只要任意分层文件是 `partial` 或 `missing`，只能输出 `max-artifact-incomplete:full-source-exhaustive-pass-not-satisfied`，不得宣称 `max-complete`。
...
- 降档、撤回或登记 `max-artifact-incomplete:<registered-reason>`。
- 不得补写强判断。
