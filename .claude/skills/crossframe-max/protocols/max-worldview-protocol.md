...
- 只在聊天窗口输出 `max-dossier` 或 `max-essay`。
- 写“短答 / 压缩版 / 摘要版 / 先给结论”后宣布 max 完成。
- 缺少 `max-artifact-manifest.md` 或四个核心产物文件。
- 只生成一个合并文件，却没有独立 `max-dossier.md`、`max-essay.md`、`max-continuation-ledger.md` 和 `max-continuation-index.md`。
- 没有运行校验脚本，或脚本失败后仍宣布完成。
- 最终聊天回复只给文件链接，没有列出可继续讨论的分支。
...
1. `max-essay` 必须大于 `max-dossier`；自动校验最低阈值为 `max-essay` 可见字符数不低于 `max-dossier` 的 1.2 倍。
2. 理想比例是：`max-essay` 占整体输出 55% 到 70%，`max-dossier` 占 20% 到 30%，台账和续写索引占 10% 到 15%。
3. 如果 `max-essay` 小于 `max-dossier`，输出只能标为“结构底稿完成，完整解释正文未完成”，不得宣布 max 完成。
4. `max-essay` 不能只是 dossier 的摘要；它必须连续展开局部世界如何运行、问题如何形成、路径如何演化、处理如何改变结构、哪些只是伪修复、反例如何撤回解释、未知如何保留。
5. 如果单轮无法写足完整正文，`max-continuation-index` 的下一轮入口必须指向“继续扩写 max-essay 的完整解释层”，而不是重新开始 dossier。
