...
读取 `../crossframe-suite/references/output-mode-selector.md`，根据用户触发词自动判定本次的 `analysis_mode`（保守/客观/激进/批判）、`role`（学术专家/实践工匠/战略决策者/大众传播/批判反思者/未来探索者）和 `topic_sensitivity`（low/normal/vulnerable/high-stakes）。

若用户在本次对话中未使用任何模式或角色触发关键词，也没有给出 `2+1` 这类数字组合，AI 必须完整渲染 `../crossframe-suite/templates/mode-selection-dialog.md` 的 4 个输出模式和 6 个角色选项，并在此处停止等待用户回复后再继续；不得只写“请选择模式/角色”或省略默认推荐。默认值（客观 + 学术专家）仅在用户明确回复“默认”“直接开始”“随便”“都行”“不用选”等放弃选择的措辞时生效。
