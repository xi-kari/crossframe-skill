...
def set_header_footer(document: Document) -> None:
    ...
    run = header.add_run("跨尺度结构诊断框架 v5.0 半量化 DLC")
    ...
    run = footer.add_run("CrossFrame v5.0 DLC · 半量化审计层")
    set_run_font(run, size=9, color=MUTED)
...
def add_title_page(document: Document, source_hash: str, release_label: str | None) -> None:
    ...
    run = title.add_run("跨尺度结构诊断框架 v5.0 半量化 DLC")
...
def build_docx(repo: Path, output_path: Path, release_label: str | None) -> None:
    ...
    document.core_properties.title = "跨尺度结构诊断框架 v5.0 半量化 DLC"
    document.core_properties.subject = "半量化结构判断系统发布稿"
    document.core_properties.author = "CrossFrame Skill Suite"
    document.core_properties.keywords = "CrossFrame, v5.0 半量化 DLC, 半量化, 结构诊断"
    ...
    toc_headings.extend(["附录 A：v5.0 与半量化 DLC 变更说明", "附录 B：v5.0 半量化 DLC 工具原型说明"])
    ...
    document.add_paragraph("附录 A：v5.0 与半量化 DLC 变更说明", style="Heading 1")
    render_markdown(document, appendix_a, shift=0, skip_first_h1=True)

    appendix_b = read(repo / DOCX_SOURCE_FILES[2])
    document.add_section(WD_SECTION.NEW_PAGE)
    document.add_paragraph("附录 B：v5.0 半量化 DLC 工具原型说明", style="Heading 1")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the CrossFrame v5.0 DLC publishable DOCX from tracked Markdown sources.")
    parser.add_argument("--repo", default=".", help="Repository root.")
    parser.add_argument("--output-dir", default="outputs/doc", help="Directory for generated DOCX output.")
    parser.add_argument("--filename", default="跨尺度结构诊断框架v5.0半量化DLC.docx", help="Output DOCX filename.")
