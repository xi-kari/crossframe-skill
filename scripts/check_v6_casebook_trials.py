...
REQUIRED_CASEBOOK_FILES = [
    "skills/crossframe-casebook/references/v6-casebook-validation-protocol.md",
    "skills/crossframe-casebook/templates/v6-quant-case-trial-template.md",
    "skills/crossframe-casebook/templates/v6-rater-disagreement-record-template.md",
    "skills/crossframe-casebook/examples/v6-quant-trials/organization-case-trial.md",
    "skills/crossframe-casebook/examples/v6-quant-trials/relationship-case-trial.md",
    "skills/crossframe-casebook/examples/v6-quant-trials/public-dispute-case-trial.md",
    "skills/crossframe-casebook/examples/v6-quant-trials/misuse-counterexample-trial.md",
    "skills/crossframe-casebook/examples/v6-quant-trials/rater-disagreement-sample.md",
...
def check_required_files(repo: Path, errors: list[str]) -> None:
    for rel in REQUIRED_CASEBOOK_FILES:
        if not (repo / rel).exists():
            add_error(errors, rel, "missing required v6 casebook validation file")
...
def check_trials(repo: Path, errors: list[str]) -> None:
    ...
    if missing_domains:
        add_error(errors, "skills/crossframe-casebook/examples/v6-quant-trials", f"missing domains: {', '.join(missing_domains)}")
    ...
    if len(formal_or_counterexample) < 3:
        add_error(errors, "skills/crossframe-casebook/examples/v6-quant-trials", "need at least three formal or counterexample trials")
    ...
    if not writeback_trials:
        add_error(errors, "skills/crossframe-casebook/examples/v6-quant-trials", "need at least one downgrade, anchor revision, or template revision")
    ...
    if len(embedded_raters) < 2:
        add_error(errors, "skills/crossframe-casebook/examples/v6-quant-trials", "need at least two formal trials with embedded rater_a/rater_b readings")


def check_disagreement(repo: Path, errors: list[str]) -> None:
    rel = "skills/crossframe-casebook/examples/v6-quant-trials/rater-disagreement-sample.md"
    path = repo / rel
    if not path.exists():
        return
    text = read(path)
...
def check_summary(repo: Path, errors: list[str]) -> None:
    rel = "skills/crossframe-casebook/examples/v6-quant-trials/validation-summary.md"
    path = repo / rel
    if not path.exists():
        return
    text = read(path)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check CrossFrame v6 casebook validation trials.")
    ...
    print("ok: v6 casebook validation trials passed")
    return 0
