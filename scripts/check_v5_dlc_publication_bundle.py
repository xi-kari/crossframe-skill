...
from build_v5_dlc_publication_bundle import BUNDLE_SOURCE_FILES, PROTOTYPE_AUDIT_FILES, SOURCE_FILES, sha256
...
def check_tracked_sources(repo: Path, errors: list[str]) -> None:
    for rel in SOURCE_FILES:
        path = repo / rel
        if not path.exists():
            add_error(errors, rel, "missing v5 DLC publication source")

    doc_rel = "docs/CROSSFRAME_V5_DLC.md"
...
def build_default_bundle(repo: Path) -> Path:
    ...
    subprocess.run(
        [
            sys.executable,
            str(repo / "scripts" / "build_v5_dlc_publication_bundle.py"),
            "--repo",
            str(repo),
            "--stamp",
            stamp,
    ...
    return repo / "outputs" / f"crossframe-v5-dlc-publication-{stamp}.md"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check CrossFrame v5 DLC publication source and generated bundle.")
    ...
    if errors:
        ...
        return 1
    print("ok: v5 DLC publication bundle checks passed")
    return 0
