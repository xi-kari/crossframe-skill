...
CrossFrame 是一组给 AI 用的中文 skills。日常使用时，推荐默认从 `crossframe-suite` 进入。

它们的作用很简单：**不要让 AI 一上来就套概念，而是先读取必要概念、把问题拆清楚，再给一个普通人能读懂的判断或文章。**

这一版又补了一层“2.0 源结构连续性”：CrossFrame 不把 `跨尺度结构诊断框架v2.0.docx` 整篇粗暴塞进上下文，但会保存它的章节脊柱、逐节摘要和必须联读的连续板块。这样拆成 skill 以后，AI 不会只读一张概念卡就下判断。

仓库里现在是一组平行 skill，复杂任务先由 `crossframe-suite` 调度：

- `crossframe-suite`：推荐默认入口，用于判断一个任务应该连续触发哪些 skill，以及按什么顺序触发。开放式分析默认输出可读文章；明确要评审、案例库、备忘录、表格或行动清单时，不强行成文。
...
| Skill | 主要功能 | 默认输出 | 什么时候用 |
| --- | --- | --- | --- |
| `crossframe-suite` | 总入口，决定连续触发哪些 skill，以及哪些不触发。 | 调度提纲 + 最终产物；开放式分析默认生成可读文章。 | 日常默认入口；用户说“分析一下”“怎么看”“讲讲”“写一下”，或一个任务要诊断、成文、评审连续完成。 |
| `crossframe` | 做结构诊断，先分清事实、尺度、证据、责任和机制候选。 | 推理提纲 + 人话判断 / 开放断言 / 推演 / 低条件行动。 | 只想分析一个关系、团队、组织、制度或公共争议，不一定要写成文章。 |
| `crossframe-essay` | 把结构判断写成中文批判性洞察文章。 | 结构洞察底稿 + 文章正文。 | 想要文章、评论、思想文章、长答复，或想把分析写得可读、能传播。 |
| `crossframe-review` | 审查输出有没有真的推理。 | 评审报告 / 是否合格 / 修复建议。 | 检查某段 AI 输出、文章或诊断是否有概念堆砌、伪推理、证据越界、跳过底稿。 |
| `crossframe-dialogue` | 写短而有洞察的答复。 | 编辑回信 / 咨询式短答复 / 意见回复。 | 用户问“我该怎么看”“是不是我想多了”“怎么回答这个读者”，但不想要长文。 |
| `crossframe-casebook` | 把材料整理成可复用案例。 | 案例库条目 / 案例索引 / 脱敏来源台账。 | 有聊天记录、复盘材料、项目材料、公共争议材料，想沉淀成以后可复用的案例。 |
| `crossframe-public` | 分析公共议题和制度问题的证据边界。 | 公共议题诊断 / 证据边界摘要 / 公共评论底稿。 | 平台申诉、政策、制度、机构合规、公共承诺、真实公共争议；通常需要查源。 |
| `crossframe-org` | 做团队、项目、组织修复专项。 | 组织诊断备忘录 / 反馈写回方案 / 复盘改造建议 / 低风险试点。 | 项目失败、复盘失真、中层耗竭、授权链断裂、责任链不清。 |
| `crossframe-teach` | 把 CrossFrame 概念讲给普通人。 | 概念课 / 误读边界 / 现实信号 / 小练习。 | 想解释“承接”“回流”“开放断言”“证据成本”等概念，而不是做完整诊断。 |
| `crossframe-debate` | 检验一个命题或争论。 | 正反方结构 / 隐藏前提 / 最强反驳 / 撤回条件。 | 用户提出一个判断、论点、争议命题，想看它能不能站住。 |
| `crossframe-notebook` | 做读书、理论、文章研究笔记。 | 研究笔记 / 来源台账 / 关联与不同 / 可吸收与冲突处。 | 读一本书、一篇文章、一种理论，想看它和 CrossFrame 的关系，而不是直接套框架。 |

最简单的记法：**默认用 `crossframe-suite`；只要某个固定产物时，才直接用对应专项 skill。**
...
很多时候你不用特意说“请生成文章”。如果你从 `crossframe-suite` 进入，只是说“分析一下这个问题”“怎么看这个现象”“讲讲这个主题”，并且没有指定要评审报告、案例库、备忘录、表格或行动清单，它会默认走文章链路：先诊断，再生成 `结构洞察底稿`，最后写成可读文章。
...
如果使用 `crossframe-suite`，它还会先给一个更短的 `调度提纲`：本次走哪些 skill、哪些不走、最后是否过评审。开放式分析默认会进入 `crossframe-essay`，输出一篇可读文章或文章式回答。
...
也不是所有任务都要生成文章。下面这些情况会保留原交付物：评审报告、案例库条目、组织修复备忘录、反馈写回方案、命题辩论表、概念教学练习、来源台账、表格、清单、一句话结论、低条件行动方案或纯诊断。

如果文章需要更深，它还会再加一层：从现实机制里抽象出上位概念，再选择贴切的经典、理论、历史经验或文学互文，最后回到现实责任链。它不会为了显得高级而堆名人名言；直接引用必须能核验，不确定原句时只做意译或思想映射。

如果文章需要更亲切的回答感，它还可以进入“现代编辑同志口吻”：先接住读者的困惑，再共同分析结构，必要时严厉批评责任转嫁、伪修复和表演性流程，最后给出清醒、稳妥、有分寸的意见。这里的“同志”不是复古口号，而是一种认真同读者谈问题的姿态。
...
2.0 原文里有些内容是连续展开的，不能拆开读。比如开放断言要和强判断、命题验证、撤回条件一起看；爱要和亲密关系、边界、修复责任一起看；公共制度要和反俘获、弱信号安全、程序有效性一起看。

仓库里现在有三层材料防止读少：

- `v2-source-spine.md`：保存 2.0 DOCX 的章节顺序和相邻关系。
- `v2-section-digest-index.md`：给每节一个保真摘要和误读边界。
