...
name: crossframe-v5-org
description: 经由 crossframe-v5-suite 调度使用，不独立响应。团队、项目、组织修复专项 skill。
metadata:
  trigger: suite-only
---
...
如果组织修复判断之后要写文章、沉淀案例、做辩论或评审输出，先读取 `../crossframe-v5-suite/SKILL.md` 做总调度；本 skill 只负责团队、项目和组织修复专项。

CrossFrame v5 Org 是 `crossframe-v5` 的平行组织修复 skill，不替代 canonical `crossframe-v5`，也不复制 CrossFrame v5 全文。它把 CrossFrame v5 的事实闸、尺度闸、责任闸、机制候选和概念保真，转成团队、项目、组织场景里的可执行修复备忘录。
...
2. 读取 `../crossframe-v5/SKILL.md`。
3. 读取 `../crossframe-v5/references/read-routing-map.md`，确定本次需要加载的 canonical protocol、worksheet、concept card 和模板。
4. 如果组织判断触发高责任、公共制度、亲密关系、长期演化、框架治理、AI 现实验证、弱信号/不透明、无法退出、工具化、隐喻/来源透明或文章输出，必须追加读取 `../crossframe-v5/references/v5-continuity-bundles.md`，并按需使用 `../crossframe-v5/worksheets/source-continuity-check.md`；未完成联读时只能降档。
...
8. 如果判断使用高风险 CrossFrame v5 概念，按 `../crossframe-v5/references/read-routing-map.md` 读取对应概念卡，并用 `../crossframe-v5/worksheets/concept-fidelity-check.md` 做概念保真检查。
...
- 输出不是文章；不要走 `crossframe-v5-essay`，除非用户明确要求写文章。
