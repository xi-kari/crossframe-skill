...
用于用户要求解释某个概念、思想、哲学立场、社会现象，或问“用 CrossFrame v5 怎么看 X”。
...
遇到哲学、意义、第一因、生命、虚无主义等抽象问题时，不要第一反应只说“CrossFrame v5 不可诊断”。先尝试把问题转成可解释的结构问题：

1. 尺度拆分：至少区分经验/科学层、结构/形式层、意义/存在层；必要时再加公共/制度层。
2. 边界声明：说明 CrossFrame v5 不裁决终极本体、神学真理或科学定论。
...
> 从 CrossFrame v5 看，X 暂时可以理解为：某种过程第一次形成了（边界/反馈/承接/写回/继承/行动能力）结构。这个判断不是终极答案，而是一个结构解释靶点。

禁止把“没有一手证据”直接等同于“不能形成任何结构解释”。没有案例证据时不能强诊断，但仍可做概念解释和结构性开放断言。

## 2. 白话解释

先用普通语言说明这个概念通常在说什么。第一段不使用 CrossFrame v5 术语。
...
- 不要把概念解释写成价值宣判。
- 不要把外部思想强行改造成 CrossFrame v5 原生概念。
- 不要因解释概念而跳过事实边界。
- 不要用一串术语替代白话解释。
