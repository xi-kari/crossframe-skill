const demos = {
  philosophy: {
    input: "/crossframe-suite 2+1，生命的第一因是什么？",
    workflow: "crossframe -> crossframe-essay -> crossframe-review -> crossframe-inquiry",
    output: [
      "拆成科学层、结构层和意义层。",
      "使用 open_assertion，不做终极裁决。",
      "把标题、中心判断和结尾回指 claim_id。"
    ],
    review: "如果正文把开放断言写成终局答案，review 会要求降档或重写。",
    inquiry: "继续追问：第一因问题在哪些条件下只能作为意义问题，而不能作为科学结论？"
  },
  history: {
    input: "/crossframe-suite 分析苏联的整个始末",
    ...
    review: "如果一个来源族被写成完整史料闭包，review 会要求补证或降档。",
    inquiry: "继续追问：哪些阶段解释需要分开，哪些结论只能保留为机制候选？"
  },
  org: {
    input: "/crossframe-suite 为什么这个团队越复盘越失真？",
  ...
  public: {
    input: "/crossframe-suite 一个平台的申诉入口是否等于治理有效？",
    ...
    review: "如果把入口可见写成治理有效，review 会标记为正文强于台账。",
