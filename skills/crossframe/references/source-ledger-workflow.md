...
| 字段 | 写法 |
| --- | --- |
| source_id | S1 / S2 / HSL1 等稳定编号，供 claim ledger 和 review 回指 |
...
| 支持的 claim_id / 命题 | 只写它直接支持的事实、背景或 claim_id，不写它不能证明的结论 |
| 不能证明什么 | 写清不能证明动机、本质、全局有效性、责任归属或强判断的部分 |
| 证据档位 | 已核验事实 / 高成本证据 / 中成本线索 / 低成本声明 / 弱信号 / 热度信号 / 解释判断 |
| 使用位置 | 底稿事实边界 / 机制候选 / claim_id / 反例 / 边界段 / 现实案例 / 正文自然提及 / 仅作待核验 |
...
- 十个字段必须全部出现；不得把 `source_id`、`支持的 claim_id / 命题`、“降档理由”和“仍需补证处”合并成一列。
...
- `source_id` 必须稳定，能被 claim ledger 和 review 回指；无法编号时不得判定为来源完整。
