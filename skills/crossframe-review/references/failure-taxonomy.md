...
修复：回到 `crossframe-suite` 的声口传递规则，把由角色和 `topic_sensitivity` 判定的 `voice_mode` 传给 `crossframe-essay`；补写 `正文声口方案`，再重写第一段，让它先接住问题而不是先展示术语链。
...
信号：文章、公共判断、高责任判断、行动建议、组织处置、强机制句或高风险概念判断已经输出，但没有 `claim ledger`，或底稿没有列出中心命题、机制候选、高风险概念、行动建议、文章转译和公共定性的 `claim_id`。

影响：正文可以直接从术语、技法、隐喻或概念感生成，review 无法复核判断来自事实、机制候选、概念契约、判断档位和撤回条件。

修复：回到 `../crossframe/templates/claim-ledger.md` 和 `../crossframe/worksheets/claim-ledger-check.md` 补台账；补不出时删除、降档，或标为“本文推断 / 表达转译 / 外部思想映射”。没有台账时不得判 `substantive_pass`。

## 正文裸奔命题

信号：正文中承担判断作用的中心句、机制句、高风险概念句、行动建议、公共定性、概率排序或点睛句没有对应 `claim_id`，或该 `claim_id` 无法回指事实、源锚点、概念契约、来源台账、判断档位和撤回条件。

影响：文章看起来完成了 CrossFrame 风格表达，但判断实际上没有交账，最容易把开放断言写成强判断，把表达技法写成证明。

修复：抽取 3-5 个正文短摘逐句回指；没有 `claim_id` 的句子必须删除、补台账、降档，或标为表达转译。涉及名誉、权利、资源、处分或公共记忆时，直接进入高责任失败处理。

## 生成层自我盖章

信号：core、essay、suite 或 sibling skill 在未进入 review 前写“质量闸通过”“完全通过”“A档”“合格”或 `substantive_pass`，或把自检摘要写成最终通过结论。

影响：生成层把候选产物伪装成已审产物，review 的反向否决、抽句回指、来源台账和概念契约检查被跳过。

修复：删除通过结论，改为“生成层自检摘要，待 review 判定”；进入 `crossframe-review` 后重新执行反向否决最小块。

## 概念契约伪未触发

信号：正文出现 `open_assertion`、开放断言、可撤回判断、最高开放断言、不能终局裁决、当前只能说、不能证明什么或撤回条件，却写“概念契约不触发”，或没有读取 `contract-open_assertion`。

影响：开放断言绕过退场、降档和行动上限，容易把可撤回判断写成事实闭合或处置依据。

修复：补读 `contract-open_assertion` 与开放断言相关联读包；不能通过时删除、降档或标为表达转译。

## 命题台账摘要冒充完整台账

信号：前台表只列 claim_id、短摘或判断档位，缺少 `source_anchor`、`mechanism_id`、`concept_contract`、`source_ledger_id` 或 `publish_boundary`，却称为完整 `claim ledger`。

影响：review 会把摘要 smoke check 当作完整可审计台账，无法判断机制、来源、概念契约和发布边界是否真的成立。

修复：把前台表改名为“命题台账摘要”，或补齐完整台账字段；缺字段前不得判 `substantive_pass`。

## 标题/点睛句强于台账

信号：标题、首段、结尾或点睛句比对应 `claim_id` 更强，提前完成强判断、公共定性、行动建议或终局裁决。

影响：正文主体即使有边界，读者先接收的标题/点睛句仍然越过了事实、来源和判断档位。

修复：弱化标题、首段、结尾和点睛句，或补足对应 `claim_id`、来源台账和发布边界；补不出时降档或改为表达转译。

## 标题/结论强于台账

信号：标题、首段、结论、结尾或行动摘要强于 `claim_id` 的判断档位、行动上限或发布边界，尤其把 `open_assertion` 写成已证实结论。

影响：正文内部即使有边界，读者接收的入口和收束仍然完成了越级判断。

修复：改写标题和结论，使其不强于台账；补不出对应 `claim_id`、source_anchor、概念契约和撤回条件时，必须降档或删除。

## 强句无 claim_id

信号：中心句、机制句、标题、结论、行动建议或公共定性承担判断作用，但没有对应 `claim_id`，或 `claim_id` 无法回指事实、源锚点、概念契约、判断档位和撤回条件。

影响：输出看起来像完成了 CrossFrame，但强判断没有交账；review 无法判断该句来自证据还是来自表达惯性。

修复：抽句回指；补完整 `claim ledger` 字段。补不出时删除、降档，或标为“本文推断 / 表达转译 / 外部思想映射”。

## 开放断言伪装成强判断

信号：台账或正文写 `open_assertion`、当前只能说、不能终局裁决、可撤回判断，但标题、结论、行动建议或公共定性却写成事实闭合、处置依据或发布结论。

影响：开放断言被捕获成强判断，撤回条件和反例入口失效。

修复：把强句降回开放断言，前置不能证明什么、撤回条件和行动上限；若用于处置、资源、名誉或公共记忆，直接阻断。

## v5 DLC 分数处置化

信号：把七闸分值、构念读数、案例库一致性、评分者一致性或 checker 通过当作处分、排名、资格、封禁、公开定性、发布通过、组织健康证明或 `substantive_pass` 的依据。

影响：半量化从“降档和补证工具”变成自动裁决工具，绕开 claim ledger、source_anchor、概念契约、受影响位置反馈和 review。

修复：删除分值处置依据；把分值改回结构剖面，只允许触发降档、补证、反例入口、行动上限收窄或 `block_publication`。高责任场景直接 F。

## 半量化自动外显

信号：普通关系、组织复盘或公共议题在用户未要求量化审计时自动输出分数；或输出没有登记 `score_visibility`，把内部评分剖面当作前台结论。

影响：用户会把分值理解成对象评价，低权力主体、亲密关系安全和公共记忆风险被放大。

修复：删除自动外显分数，登记 `score_visibility: hidden / profile_only / user_requested_profile`；若用户明确要量化，只展示结构剖面、证据缺口、不能证明什么、行动上限和撤回条件。
...
信号：公共评论、批判文章、真实机构/平台/政策/人物/公司、最新事实、AI/过程性产物或强判断已经使用外部材料，但没有逐条记录source_id、来源、时间、来源类型、支持的 claim_id / 命题、不能证明什么、证据档位、使用位置、降档理由、仍需补证处。
...
信号：台账表面有列，但十字段缺项、合并“降档理由/仍需补证处”、用“官方页面/官方列表/机构网站”等来源描述填时间，或 `使用位置` 不能定位到正文标题、段落、短摘或命题强度。
...
信号：AI 材料、弱信号、过程性产物、搜索摘要、热度或平台自评被写成高成本证据，支撑公共定性、组织处置、强机制句或强判断。

影响：弱信号被包装成强证据，容易服务于权力封闭。

修复：把 AI 材料降为待审文本；要求原始记录、独立证据、可申诉通道、反向条件和行动上限。
...
修复：先读 `runtime-read-policy.md`、`continuity-closure-map.md` 和对应连续联读包；只有源锚点不足或需要源审计时，才定向补读 `v5-source-spine.md`、`v5-section-digest-index.md`、`v5-material-selection-map.md` 的相关局部。用 `source-continuity-check.md` 重做降档判断。
