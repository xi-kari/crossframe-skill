...
-> revalidate
-> pass / downgrade / withdraw / mark_artifact_incomplete
```
...
超过 retry budget 后必须登记 `mark_artifact_incomplete`，并由失败报告按 `max-validation-failed:`、当前 profile、`retry_budget_exhausted` 三段生成标签。
