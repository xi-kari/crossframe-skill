...
- 说明 full-source exhaustive pass 状态：完成则写 `satisfied` 和 `3273 / 3273`；未完成则写 source frontier、未读分层、受影响 claim 和下一轮补读计划。
...
产物清单必须包含：phase-lock gate；max-run-contract.json；max-read-plan.json；max-source-snapshot.json；max-worldview-capsule.locked.md；max-local-world-model.locked.md；max-claim-board.json；max-audit-board.json；max-output-plan.locked.md；full-source exhaustive pass 状态；total paragraphs: 3273 / 3273 状态；max-full-source-read-ledger；max-read-ledger.json；max-claim-ledger.json；max-concept-hit-ledger.json；max-evidence-reasoning-audit.json；max-validator-report.json（如已运行）；max-repair-plan.json（如本轮发生 validation failure）；read status: full / partial / missing；stage 8 final read audit；max-artifact-incomplete 原因（如未达 max-complete）。
```
...
- 禁止把 `max-dossier` 当正文。
- 禁止因为 full-source、结构化台账或 validator 未完成而取消 `max-essay.md`；这些只阻断 `max-complete` 宣称。
...
- 禁止在触发 retrieval-trigger-policy 时跳过主动检索或反向检索；如果检索失败，必须登记失败并降档相关 claim。
- 禁止在 full-source exhaustive pass 未满足时宣称 `max-complete`。
- 禁止让宏观解释取消低尺度事实和责任链。
- 禁止把演化路径写成命运预言。
- 禁止把处理问题写成处置授权。
- 禁止把未找到证据写成证据不存在。
