...
这条默认不再统一启用 `crossframe-essay` 的现代编辑底色。声口由 `references/output-mode-selector.md` 的角色和 `topic_sensitivity` 共同决定：学术专家/批判反思者默认中性分析体，大众传播/未来探索者可启用编辑底色；用户显式要求"亲切/编辑口吻"时覆盖。中性分析体不是冷淡体，`vulnerable` 主题仍要先接住人。
