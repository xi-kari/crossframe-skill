...
v6.0 工具原型是一组 Markdown 模板、JSON schema、fixtures、校验脚本和发布 bundle 脚本。它用于整理结构判断和暴露风险，不用于自动判断现实。

## 包含内容

| 类型 | 文件 |
| --- | --- |
| 构念图谱 | `skills/crossframe/references/construct-map-v6.md` |
| 七闸半量化 | `skills/crossframe/worksheets/seven-gates-quant-rubric.md` 与 `skills/crossframe/schemas/seven-gates-quant.schema.json` |
| 证据台账 | `skills/crossframe/worksheets/evidence-ledger-v6.md` 与 `skills/crossframe/schemas/evidence-ledger-v6.schema.json` |
...
| 案例库验证 | `skills/crossframe-casebook/examples/v6-quant-trials/` |
| 发布源稿 | `docs/CROSSFRAME_V6.md` |
| 构建脚本 | `scripts/build_v6_publication_bundle.py` |
| DOCX 导出 | `scripts/build_v6_docx.py` |
| 发布检查 | `scripts/check_v6_publication_bundle.py` |
| 审计 manifest | 发布 bundle manifest 会记录文档源、工具脚本、v6 validators 和 schema 的 SHA256 |
