# CrossFrame v5 DLC Quantification Foundation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the first testable v5.0 半量化 DLC half-quantification foundation: schemas, fixtures, Markdown worksheets, guardrail validation, and integrity-check wiring.

**Architecture:** Keep v5.0 半量化 DLC quantification as a bounded audit layer inside `skills/crossframe/`, not a new scoring product. JSON schemas define allowed structured records, Markdown worksheets keep the Chinese semantic contract visible, and a validator rejects total-score keys, missing withdrawal conditions, missing action ceilings, and strong-judgment records without source ledgers. Existing CrossFrame integrity checks remain the outer gate.
...
This plan implements Phase 2 from `docs/superpowers/specs/2026-06-30-crossframe-v5-dlc-quantification-design.md`.

This plan does not implement casebook trial runs, the final v5.0 半量化 DLC publication document, DOCX generation, public site copy, or release packaging. Those need separate plans after this foundation is green.

## File Structure

Create these files:

- `skills/crossframe/references/construct-map-v5-dlc.md` defines the first v5 DLC construct set and forbidden merges.
- `skills/crossframe/worksheets/seven-gates-quant-rubric.md` is the user-facing seven-gate half-quantification worksheet.
- `skills/crossframe/worksheets/evidence-ledger-v5-dlc.md` extends the evidence ledger with v5 DLC fields.
...
- `skills/crossframe/schemas/evidence-ledger-v5-dlc.schema.json` validates v5 DLC evidence-ledger records.
...
- `skills/crossframe/schemas/fixtures/v5-dlc/<schema-id>/valid-*.json` and `invalid-*.json` prove every schema gate.
- `scripts/validate_v5_dlc_quantification_schema_fixtures.py` validates every v5 DLC schema fixture and applies cross-schema semantic guardrails.

Modify these files:

- `scripts/check_crossframe_skill_integrity.py` adds a `check_v5_dlc_quantification_foundation()` function and calls it from `check_root()`.
- `docs/QUICKSTART.md` adds the v5 DLC validator command to local verification.
- `README.md` mentions the v5 DLC quantification foundation as a guarded internal audit layer, not a scoring system.
...
Forbidden keys anywhere in valid v5 DLC fixtures:
...
## Task 1: Add V5 DLC Fixture Validator

**Files:**
- Create: `scripts/validate_v5_dlc_quantification_schema_fixtures.py`
- Test: `skills/crossframe/schemas/fixtures/v5-dlc/**`
...
Expected: FAIL because `scripts/validate_v5_dlc_quantification_schema_fixtures.py` does not exist.
...
TARGETS = [
    SchemaTarget(
        ...
        Path("skills/crossframe/schemas/fixtures/v5-dlc/seven-gates-quant"),
    ),
    SchemaTarget(
        "evidence-ledger-v5-dlc",
        Path("skills/crossframe/schemas/evidence-ledger-v5-dlc.schema.json"),
        Path("skills/crossframe/schemas/fixtures/v5-dlc/evidence-ledger-v5-dlc"),
    ),
    SchemaTarget(
        "calibration-anchor",
        Path("skills/crossframe/schemas/calibration-anchor.schema.json"),
        Path("skills/crossframe/schemas/fixtures/v5-dlc/calibration-anchor"),
    ),
    SchemaTarget(
        "mechanism-update",
        Path("skills/crossframe/schemas/mechanism-update.schema.json"),
        Path("skills/crossframe/schemas/fixtures/v5-dlc/mechanism-update"),
    ),
    SchemaTarget(
        "counterexample-register",
        Path("skills/crossframe/schemas/counterexample-register.schema.json"),
        Path("skills/crossframe/schemas/fixtures/v5-dlc/counterexample-register"),
    ),
]
...
def validate_target(repo: Path, target: SchemaTarget) -> int:
    ...
    if not schema_path.exists():
        raise SystemExit(f"missing v5 DLC schema: {schema_path}")
    if not fixture_dir.is_dir():
        raise SystemExit(f"missing v5 DLC fixture directory: {fixture_dir}")

    schema = load_json(schema_path)
    validator = Draft202012Validator(schema)
    fixtures = sorted(fixture_dir.glob("*.json"))
    if not fixtures:
        raise SystemExit(f"missing v5 DLC fixtures: {fixture_dir}")
    ...
    for fixture in fixtures:
        ...
        if should_pass and errors:
            ...
            raise SystemExit(f"valid fixture failed v5 DLC validation: {fixture.name}\n{details}")
        if should_fail and not errors:
            raise SystemExit(f"invalid fixture unexpectedly passed v5 DLC validation: {fixture.name}")
        checked += 1
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate CrossFrame v5 DLC quantification schema fixtures.")
    ...
    print(f"ok: v5 DLC quantification schema fixtures validated: {total}")
    return 0
...
Expected: FAIL with `missing v5 DLC schema: ...seven-gates-quant.schema.json`.
...
```powershell
git add scripts/validate_v5_dlc_quantification_schema_fixtures.py
git commit -m "test: add v5 DLC quantification fixture validator"
```
...
- Create: `skills/crossframe/schemas/fixtures/v5-dlc/seven-gates-quant/valid-seven-gates-profile.json`
- Create: `skills/crossframe/schemas/fixtures/v5-dlc/seven-gates-quant/invalid-total-score.json`
- Create: `skills/crossframe/schemas/fixtures/v5-dlc/seven-gates-quant/invalid-fail-allows-strong.json`
...
Expected: FAIL on the next missing schema, `evidence-ledger-v5-dlc.schema.json`.
...
```powershell
git add skills/crossframe/schemas/seven-gates-quant.schema.json skills/crossframe/schemas/fixtures/v5-dlc/seven-gates-quant skills/crossframe/worksheets/seven-gates-quant-rubric.md
git commit -m "feat: add v5 DLC seven-gates quant schema"
```

## Task 3: Add Evidence Ledger V5 DLC Schema And Worksheet

**Files:**
- Create: `skills/crossframe/schemas/evidence-ledger-v5-dlc.schema.json`
- Create: `skills/crossframe/schemas/fixtures/v5-dlc/evidence-ledger-v5-dlc/valid-evidence-ledger.json`
- Create: `skills/crossframe/schemas/fixtures/v5-dlc/evidence-ledger-v5-dlc/invalid-strong-without-source-ledger.json`
- Create: `skills/crossframe/schemas/fixtures/v5-dlc/evidence-ledger-v5-dlc/invalid-claim-without-withdrawal.json`
- Create: `skills/crossframe/worksheets/evidence-ledger-v5-dlc.md`
...
`evidence-ledger-v5-dlc.md` must include fields for evidence cost, directness, independence, verifiability, counterevidence status, diagnostic force, mechanism edge, downgrade reason, withdrawal trigger, action ceiling, and cannot-prove boundary.
...
```powershell
git add skills/crossframe/schemas/evidence-ledger-v5-dlc.schema.json skills/crossframe/schemas/fixtures/v5-dlc/evidence-ledger-v5-dlc skills/crossframe/worksheets/evidence-ledger-v5-dlc.md
git commit -m "feat: add v5 DLC evidence ledger schema"
```
...
- Create: `skills/crossframe/schemas/fixtures/v5-dlc/calibration-anchor/valid-calibration-anchor.json`
- Create: `skills/crossframe/schemas/fixtures/v5-dlc/calibration-anchor/invalid-missing-misuse-risk.json`
- Create: `skills/crossframe/schemas/fixtures/v5-dlc/calibration-anchor/invalid-totalizing-construct.json`
- Create: `skills/crossframe/worksheets/calibration-anchor-card.md`
- Create: `skills/crossframe/references/construct-map-v5-dlc.md`
...
`construct-map-v5-dlc.md` must include the first ten constructs from the design spec:
...
```powershell
git add skills/crossframe/schemas/calibration-anchor.schema.json skills/crossframe/schemas/fixtures/v5-dlc/calibration-anchor skills/crossframe/worksheets/calibration-anchor-card.md skills/crossframe/references/construct-map-v5-dlc.md
git commit -m "feat: add v5 DLC calibration anchors"
```
...
- Create: `skills/crossframe/schemas/fixtures/v5-dlc/mechanism-update/valid-mechanism-update.json`
- Create: `skills/crossframe/schemas/fixtures/v5-dlc/mechanism-update/invalid-withdraw-without-counterevidence.json`
- Create: `skills/crossframe/schemas/fixtures/v5-dlc/mechanism-update/invalid-strong-judgment-without-source-ledger.json`
...
```powershell
git add skills/crossframe/schemas/mechanism-update.schema.json skills/crossframe/schemas/fixtures/v5-dlc/mechanism-update skills/crossframe/worksheets/mechanism-update-rules.md
git commit -m "feat: add v5 DLC mechanism update schema"
```
...
- Create: `skills/crossframe/schemas/fixtures/v5-dlc/counterexample-register/valid-counterexample-register.json`
- Create: `skills/crossframe/schemas/fixtures/v5-dlc/counterexample-register/invalid-missing-writeback.json`
- Create: `skills/crossframe/schemas/fixtures/v5-dlc/counterexample-register/invalid-misuse-without-action-change.json`
...
- [ ] **Step 6: Run full v5 DLC validation**
...
```powershell
git add skills/crossframe/schemas/counterexample-register.schema.json skills/crossframe/schemas/fixtures/v5-dlc/counterexample-register skills/crossframe/worksheets/counterexample-register.md
git commit -m "feat: add v5 DLC counterexample register schema"
```

## Task 7: Wire V5 DLC Foundation Into Existing Integrity Checks
...
In `scripts/check_crossframe_skill_integrity.py`, add `check_v5_dlc_quantification_foundation(root: Path, label: str) -> None` after `check_claim_ledger()`.

The function must check:

```text
all five v5 DLC schema files exist
all five v5 DLC fixture directories exist
each schema contains "$schema", "additionalProperties", and its schema-specific title
all six v5 DLC Markdown reference or worksheet files exist
...
README says v5 DLC quantification is not a total-score system
QUICKSTART mentions validate_v5_dlc_quantification_schema_fixtures.py
```

- [ ] **Step 2: Call the integrity function**

Call `check_v5_dlc_quantification_foundation(root, label)` inside `check_root()` immediately after `check_claim_ledger(root, label)`.
...
```markdown
The v5 DLC quantification foundation is an internal audit layer. It uses anchored rubrics, schema fixtures, and downgrade checks to expose uncertainty and action ceilings. It is not a total-score system, prediction model, certification system, or disposition tool.
```
...
git commit -m "chore: wire v5 DLC quantification integrity checks"
```

## Task 8: Run Full Local Verification

**Files:**
- No source edits unless a verification failure points to a v5 DLC file created by this plan.
...
- [ ] **Step 2: Validate v5 DLC fixtures**
...
Expected: PASS with one line for each v5 DLC schema target and one total line.
...
Expected: PASS. If it fails only because v5 DLC files are not yet included in source-continuity rules, record the exact failure and add a small continuity mapping in the same implementation slice.
...
- [ ] **Step 2: Search for total-score language in v5 DLC files**

Run:

```powershell
rg -n -e 'total_score' -e 'overall_score' -e 'average_score' -e 'weighted_score' -e '总分' -e '综合分' -- skills/crossframe/schemas skills/crossframe/worksheets skills/crossframe/references/construct-map-v5-dlc.md
```
